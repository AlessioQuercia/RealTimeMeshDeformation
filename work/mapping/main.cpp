...
//    }
    ...
    Shader deformShader("mapping\\shaders\\shaderNM.VERT", "mapping\\shaders\\shaderNM.FRAG"); //, "mapping\\shaders\\shaderNM.GEO");
    
    Shader normalShader("mapping\\shaders\\shaderNMFirstNormal.VERT", "mapping\\shaders\\shaderNMFirstNormal.FRAG");
    Shader displacementShader("mapping\\shaders\\shaderNMFirstDisplacement.VERT", "mapping\\shaders\\shaderNMFirstDisplacement.FRAG");
...
//    Model nanosuitModel("C:\\Users\\Alessio\\Documents\\GitHub\\Progetto_RTGP\\models\\nanosuit\\nanosuit.obj");
    ...
    // -----------------------------------------------------------------------------
    unsigned int normalMap = loadTexture("C:\\Users\\Alessio\\Documents\\GitHub\\Progetto_RTGP\\work\\mapping\\Normal.png");
    unsigned int displacementMap = loadTexture("C:\\Users\\Alessio\\Documents\\GitHub\\Progetto_RTGP\\work\\mapping\\Displacement.png");
    
    // framebuffer configuration
    // -------------------------
    unsigned int framebufferN;
    glGenFramebuffers(1, &framebufferN);
    glBindFramebuffer(GL_FRAMEBUFFER, framebufferN);
...
//                    printf("WORLD POINT B: %lf %lf %lf\n", ptB.x(),ptB.y(),ptB.z());
    ...
    //            glUniformMatrix3fv(glGetUniformLocation(objectShader->ID, "normalMatrix"), 1, GL_FALSE, glm::value_ptr(objNormalMatrix));
                ...
                if (objectShader == &deformShader)
                {
                    ...
                    // normalMap
                    objectShader->setInt("normalMap", 0);
                    if (hit)
                    {
                        objectShader->setInt("hit", 1);
                        objectShader->setVec3("hitPoint", contactPoint1);
                        objectShader->setVec3("hitDirection", camera.Front);
                    }
                }
                else
                {
            ...
            objDiffuseLocation = glGetUniformLocation(objectShader->ID, "diffuseColor");

            num_cobjs = bulletSimulation.dynamicsWorld->getNumCollisionObjects();
            ...
            for(i = 1; i < num_cobjs; i++ )
            {
                if (i <= total_cubes)
                {
                    if (i < 3)
                        ...
                        objectModel = &cubeModel;
                    objectShader = &displacementShader;
                    obj_size = cube_size;
                    glUniform3fv(objDiffuseLocation, 1, diffuseColor);
                }
                else
